...
import { AddressType, PeruseJobStatus } from '@module-persistence/entities';
...
import { Input, PeruseAddress } from '../common';
import { PeruseDocumentClassifications } from './peruse-classification-categories';
...
const getFirstElement = (input: string | string[]): string => {
  if (!input) {
    return '';
  }
...
export class RateConfirmationResult extends BaseModel<RateConfirmationResult> {
  ...
  getBrokerReferenceNumber() {
    return getFirstElement(this.brokerReferenceNumber);
  }
...
export class PeruseClassificationResult extends BaseModel<PeruseClassificationResult> {
  static logger = new Logger(PeruseClassificationResult.name);
  ...
  validate(options?: ValidationOptions) {
    if (this.status === 'error') {
      PeruseClassificationResult.logger.error(
        `Processed document has an error status`,
        {
          peruseResponse: this,
        },
      );
  ...
  getClassificationResult(): PeruseDocumentClassifications {
